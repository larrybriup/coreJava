...
public class Lockers {
	...
	*/    
    ...
    public static void 
    testLockTest() throws Exception{    
        final LockTest lockTest = 
        		new LockTest();    
        ...
        Future future = null;    
        /* 同时执行任务1三次，
         * 由于addValue方法使用了锁机制，所以，实质上会顺序执行     
         */
        for (int i=0; i<3; i++){    
            future = cachedService.
            		submit(task1);    
        }    
        // 等待最后一个任务1被执行完     
        future.get();    
     ...
     * ReadWriteLock提供的方法有：  
     * readLock(): 返回一个读的lock   
     * writeLock(): 返回一个写的lock, 
     * 此lock是排他的。  
     * ReadWriteLockTest很适合处理类似文件
     * 的读写操作。  
     * 读的时候可以同时读，但不能写；
     * 写的时候既不能同时写也不能读。  
     */    
